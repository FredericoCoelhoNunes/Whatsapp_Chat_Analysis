...
case class ChatParser () { //case class: the constructor is specified after the class name
  ...
  def parse (lines: List[String]) : List[Map[String, String]] = {
     ...
     *  Output:
     *  	List of maps each containing the parsed groups.  
     */
  ...
  def getNames (parsedMsgs : List[Map[String, String]]) : List[String] = {
    /** Gets the names of all participants */
    return (for (msg <- parsedMsgs) yield msg("name")).toList.distinct
  }
